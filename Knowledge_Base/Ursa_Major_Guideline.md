...
* **Tier 1: Strategic Allocation (The "Pool"):** Provides no-cost access to baseline compute and strategic AI services. This tier is strictly for compliant, whitelisted services (No Cloud GPUs).
* **Tier 2: Commercial Recharge (The "Override"):** Designed for high-impact projects with specialized technical requirements that fall outside the Tier 1 subsidy. **All Secure Enclaves (NIST 800-171/CMMC), Cloud GPU (A100, L4, T4), high-performance compute, and massive storage workloads must operate in this tier via direct recharge to a grant COA.** For subsidized GPU access, researchers must utilize the UCR HPCC.
